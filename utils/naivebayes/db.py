...
class Db:
    def __init__(self):
        self.conn = sqlite3.connect('./resources/private/bayes.db')
        c = self.conn.cursor()
        try:
            c.execute('drop table if exists word')
            c.execute('drop table if exists doctype_count')

            c.execute('create table if not exists word(word, doctype, count)')
            c.execute('create table if not exists doctype_count(doctype, count)')
            c.execute('create index if not exists i1 on word(word, doctype)')

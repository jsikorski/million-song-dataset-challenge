...
def create_plays_by_user_binary(plays_by_user, most_popular_songs, target_collection):
    ...
    for user_play_map in plays_by_user.find():
        ...
        for song in most_popular_songs:
            ...
            if song_index in user_play_map['value']:
                entry['value'].append(int(song_index))
...
with MongoClient('localhost', MONGODB_PORT) as client:
    ...
    invoke_measurable_task(
        lambda: create_plays_by_user_binary(db.plays_by_user_t, most_popular_songs, db.plays_by_user_simple_t),
        "Create plays_by_user_simple collection for train set")

    invoke_measurable_task(
        lambda: create_plays_by_user_binary(db.plays_by_user_v, most_popular_songs, db.plays_by_user_simple_v),
        "Create plays_by_user_simple collection for validation set")